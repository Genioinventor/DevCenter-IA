...
const AVISO_TITULO = "¡DevCenter 4.2!"; // <--- Título del aviso
const AVISO_DESCRIPCION =
    "Mejora de bugs y más inteligencia."
...
async function sendMessage(customPrompt) {
    ...
    chat.messages.push({ type: 'user', content, timestamp: Date.now() });
...
async function generateWebpage(prompt) {
    ...
    if (userMessagesCount >= 2) {
        ...
        // Obtener preferencia de tema (claro/oscuro)
...
systemPrompt = `
CREA EL MEJOR PROMT DE MUNDO SUPER ETRUCTURADO Y BIEN ECHO TARDA LO QUE TU QUIREAS: DEVCENTER AI - GENERADOR WEB INTELIGENTE 
...
📋 CONTEXTO Y PERSONALIZACIÓN (Dada por el usuario)
Año: 2025 
